...
class COCODataset(JointsDataset):
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        :return: db entry
        """
        im_ann = self.coco.loadImgs(index)[0] # 이미지를 불러옴
        width = im_ann['width']
        height = im_ann['height']

        annIds = self.coco.getAnnIds(imgIds=index, iscrowd=False) # 이미지에 있는 사람들 불러옴
        ...
        for obj in objs:
            ...
            if cls != 1: # mscoco에서 1은 사람 
                continue # 사람만 쓰므로 사람 아니면 스킵

            # ignore objs without keypoints annotation
            if max(obj['keypoints']) == 0:
                continue
            # joints_3d라고 보통 명명하지만 실제론 2d
            joints_3d = np.zeros((self.num_joints, 3), dtype=np.float)
            joints_3d_vis = np.zeros((self.num_joints, 3), dtype=np.float)
            for ipt in range(self.num_joints):
                joints_3d[ipt, 0] = obj['keypoints'][ipt * 3 + 0]
                joints_3d[ipt, 1] = obj['keypoints'][ipt * 3 + 1]
                joints_3d[ipt, 2] = 0 # 0을 넣어서 실제로 2d(항상 0 넣음)
                t_vis = obj['keypoints'][ipt * 3 + 2] # visibility 즉 annotation이 되어있는가
                if t_vis > 1: # 1보다 크면 training에 쓸 수 있다
                    t_vis = 1
            ...
            # dataloader에서 불러올 data list를 dic로 listing
            rec.append({ 
